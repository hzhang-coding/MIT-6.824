...
func (rf *Raft) CheckSendMessage(server int, term int) {
	...
	for rf.killed() == false && rf.currentTerm == term {
		...
		if interval > 300 * time.Millisecond {
			...
			cond := sync.NewCond(rf.mu.RLocker())
			rf.sendCond[server] = cond

			go func() {
				if interval > 50 * time.Millisecond {
					time.Sleep(50 * time.Millisecond - interval)
				} else {
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.sendCond = make(map[int]*sync.Cond)
