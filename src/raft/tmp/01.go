...
func (rf *Raft) sendAppendEntries(server int) {
	...
	if rf.matchIndex[server] == args.PrevLogIndex {
		n := len(rf.log[rf.nextIndex[server]:])
		if n > 100 {
			n = 100
		}
		args.Entries = make([]Log, n)
		copy(args.Entries, rf.log[rf.nextIndex[server] : rf.nextIndex[server] + n])
	}
		...
		for i := 0; i < n; i++ {
			...
			go func(server int) {
				...
				for time.Now().Before(duetime) == true {
					ok := rf.sendHeartbeat(server, term)
...
func (rf *Raft) LeaderElection() {
	...
	for i := 0; i < n; i++ {
		...
		go func(server int, args RequestVoteArgs) {
			...
			for time.Now().Before(duetime) == true {
				ok := rf.sendRequestVote(server, &args)
