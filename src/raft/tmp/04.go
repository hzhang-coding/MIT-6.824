...
func (rf *Raft) persist(saveSnapshot bool) {
	...
	if saveSnapshot == false {
		...
		/*
		ww := new(bytes.Buffer)
		ee := labgob.NewEncoder(ww)
		ee.Encode(rf.snapshot)
		snapshot := ww.Bytes()
		*/
...
func (rf *Raft) readPersist(state []byte) {
	...
	if snapshot == nil || len(snapshot) < 1 { // bootstrap without any snapshot?
        return
    }
	r = bytes.NewBuffer(snapshot)
	d = labgob.NewDecoder(r)
	var data []byte
	if d.Decode(&lastIncludedIndex) == nil && d.Decode(&data) == nil {
		rf.snapshot = data
		rf.ApplySnapshot()
	}
	*/
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	if index > rf.lastIncludedIndex {
		...
		if rf.isleader == true {
			rf.count = rf.count[index-rf.lastIncludedIndex:]
		}
...
func (rf *Raft) sendInstallSnapshot(server int) {
    ...
    rf.mu.Lock()
    if rf.isleader == false || rf.lastIncludedIndex <= rf.matchIndex[server] {
        rf.mu.Unlock()
        return
    }
    ...
    if args.LastIncludedIndex != rf.lastIncludedIndex || rf.isleader == false || args.Term != rf.currentTerm {
        rf.mu.Unlock()
        return
    }
	...
	if args.PrevLogIndex >= len(rf.log) + rf.lastIncludedIndex {
		...
		if args.PrevLogIndex < rf.lastIncludedIndex {
			...
			if rf.commitIndex < args.LeaderCommit {
				n := args.PrevLogIndex + 1 + len(args.Entries) + rf.lastIncludedIndex
...
func (rf *Raft) sendAppendEntries(server int) {
	...
	if args.PrevLogIndex != rf.nextIndex[server]-1 || rf.isleader == false || args.Term != rf.currentTerm {
		rf.mu.Unlock()
		return
	}
	...
	if reply.Success == true {
		...
		if reply.XTerm != -1 {
			i := rf.nextIndex[server] - 1 - rf.lastIncludedIndex
	...
	i := rf.matchIndex[server] - rf.lastIncludedIndex
	n := len(rf.peers)
...
func (rf *Raft) CheckSendAppendEntries(server int, term int) {
	for rf.killed() == false {
		...
		if rf.matchIndex[server] < rf.lastIncludedIndex+len(rf.log)-1 || rf.lastCommit[server] < rf.commitIndex {
		...
		if interval > 400*time.Millisecond {
			...
			if sendInstallSnapshot == true {
				rf.sendInstallSnapshot(server)
				// time.Sleep(20 * time.Millisecond)
			} else if sendAppendEntries == true {
...
func (rf *Raft) LeaderElection() {
	...
	for i := 0; i < n; i++ {
		...
		go func(server int, args RequestVoteArgs, vote map[int]int) {
			duetime := time.Now().Add(100 * time.Millisecond)
	...
	for rf.killed() == false {
		...
		if rf.currentTerm == args.Term && len(vote) > n/2 {
			rf.isleader = true
			rf.dirty = true
			exit = true
