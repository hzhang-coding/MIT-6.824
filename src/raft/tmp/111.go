...
func (rf *Raft) sendAppendEntries(server int) {
	...
	rf.mu.Lock()
	rf.lastComm[server] = time.Now()
	if args.Term != rf.currentTerm || args.PrevLogIndex != rf.nextIndex[server] - 1 {
		rf.mu.Unlock()
		return  
	}

	if reply.FollowerCommit > rf.followerCommit[server] {
		rf.followerCommit[server] = reply.FollowerCommit
	}
...
func (rf *Raft) CheckSendAppendEntries(server int, term int) {
    for rf.killed() == false {
		...
		if rf.matchIndex[server] < len(rf.log) - 1 || rf.followerCommit[server] < rf.commitIndex  {
			toSend = true
		}

		interval := time.Now().Sub(rf.lastComm[server])
        rf.mu.Unlock()
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	rf.mu.Lock()
 	if rf.state == 2 {
		...
		rf.buffer = append(rf.buffer, Log{command, rf.currentTerm})
	}
	rf.mu.Unlock()
	...
	for rf.killed() == false {
		...
		if rf.currentTerm == args.Term {
			if vote > n / 2 {
				...
				rf.log = append(rf.log, rf.buffer...)
				rf.dirty = true
