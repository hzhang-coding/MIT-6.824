...
func (rf *Raft) sendAppendEntries(server int) {
	...
	if update == true {
		go rf.UpdateCommitIndex(server)
	}
...
func (rf *Raft) UpdateCommitIndex(server int) {
	rf.mu.Lock()
	for i := rf.matchIndex[server] - rf.lastIncludedIndex; i > rf.commitIndex - rf.lastIncludedIndex && rf.log[i].Term == rf.currentTerm; i-- {
		if _, ok := rf.count[i][server]; ok == true {
			break
		}
		rf.count[i][server] = 1
        if len(rf.count[i]) > len(rf.peers) / 2 {
