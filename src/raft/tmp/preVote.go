...
func (rf *Raft) sendAppendEntries(server int) {
	...
	if rf.matchIndex[server] == args.PrevLogIndex {
		...
		args.Entries = make([]Log, len(rf.log[rf.nextIndex[server] : ]))
	...
	if rf.isleader == false || args.Term != rf.currentTerm || args.PrevLogIndex != rf.nextIndex[server] - 1 {
		rf.mu.Unlock()
		return  
	}
...
func (rf *Raft) CheckSendAppendEntries(server int, term int) {
    for rf.killed() == false {
		...
		} else if interval > 300 * time.Millisecond {
...
func (rf *Raft) sendPreRequestVote(server int, args *RequestVoteArgs) {
    reply := RequestVoteReply{}
...
func (rf *Raft) preLeaderElection() bool {
	args := RequestVoteArgs{}

	rf.mu.Lock()
	rf.preVote = make(map[int]int)
	rf.preVote[rf.me] = 1
	rf.preComplete = make(map[int]int)
	rf.preComplete[rf.me] = 1
	n := len(rf.peers)
	me := rf.me
	args.Term = rf.currentTerm
	...
	for i := 0; i < n; i++ {
		...
		go func(server int, args RequestVoteArgs) {
			...
			for time.Now().Before(duetime) == true {
				...
				rf.mu.Lock()
				_, ok := rf.preComplete[server]
				if ok == true || args.Term != rf.currentTerm {
					rf.mu.Unlock()
					return
				}
				rf.mu.Unlock()
	...
	for rf.killed() == false {
		...
		rf.mu.Lock()
		if len(rf.preComplete) == n || rf.currentTerm != args.Term {
			exit = true
		}
		if rf.currentTerm == args.Term && len(rf.preVote) > n / 2 {
			exit = true
			success = true
...
func (rf *Raft) LeaderElection() {
	...
	rf.mu.Lock()
	rf.vote = make(map[int]int)
	rf.vote[rf.me] = 1
	rf.complete = make(map[int]int)
	rf.complete[rf.me] = 1
	rf.currentTerm++
    rf.votedFor = rf.me
	n := len(rf.peers)
	...
	for rf.killed() == false {
		...
		rf.mu.Lock()
		if len(rf.complete) == n || rf.currentTerm != args.Term {
			exit = true
		}
		if rf.currentTerm == args.Term && len(rf.vote) > n / 2 {
			rf.isleader = true
            rf.dirty = true
			exit = true
