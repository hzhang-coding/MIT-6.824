...
func (rf *Raft) sendAppendEntries(server int) {
	...
	if update == true && rf.matchIndex[server] > rf.commitIndex {
        index := []int{}
        ...
        sort.Ints(index)
		commitIndex := index[len(rf.peers) / 2]
        if commitIndex > rf.commitIndex && commitIndex >= rf.beginIndex {
...
func (rf *Raft) LeaderElection(term int) {
	...
	rf.beginIndex = len(rf.log) + rf.lastIncludedIndex
