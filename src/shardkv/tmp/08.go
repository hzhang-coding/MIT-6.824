...
func (kv *ShardKV) Update(shard int) {
    for kv.killed() == false {
		...
		if kv.isleader == true && version < kv.latestVersion && kv.exist[shard] == true && kv.configs[version+1].Shards[shard] != kv.gid { 
			args := MigrateArgs{}
			args.Shard = shard
...
func (kv *ShardKV) Execute(op Op) {
	if op.Type < 3 {
		if kv.exist[op.Shard] == true && kv.versions[op.Shard] == kv.latestVersion {
		...
		if op.Config.Num == kv.latestVersion + 1 {
			...
			kv.latestVersion++
			kv.configs[kv.latestVersion] = op.Config
		}
	} else if op.Type == 4 {
		if kv.versions[op.Shard] == op.Version {
			kv.versions[op.Shard]++
			kv.exist[op.Shard] = false
			kv.database[op.Shard] = nil
			kv.clerkInfo[op.Shard] = nil
			for i := kv.versions[op.Shard]; i < kv.latestVersion; i++ {
