...
func CallHandOut(mapf func(string, string) []KeyValue,
	...
	if ok {
		...
		if reply.Type == 0 {
			...
			for i := 0; i < nReduce; i++ {
				filename := "mr-" + strconv.Itoa(reply.Num) + "-" + strconv.Itoa(i)
			...
			cargs := CompletedArgs{}
			creply := CompletedReply{}
			cargs.Type = 0
			cargs.File = filename
			creply.Flag = false

			ok := call("Coordinator.Completed", &cargs, &creply)
