...
type Op struct {
	...
	Type	int // -1: No-op, 0:Get, 1:Put, 2:Append
	Key		string
...
func (kv *KVServer) CheckState() {
	for kv.killed() == false {
		...
		kv.mu.Lock()
		if kv.isleader == false && isleader == true {
			op := Op{}
			op.Type = -1
			op.Term = term
			kv.rf.Start(op)
...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	// Your code here.
	// fmt.Printf("%v Get %v\n", args.ClerkId, args.Seq)
	kv.mu.Lock()
	// fmt.Printf("%v %v\n", kv.applyTerm, kv.term) 
	...
	if kv.applyTerm != kv.term || kv.rf.GetMajorityReply() == false {
		reply.Err = "Waiting"
		kv.mu.Unlock()
		return
	}
	...
	// kv.clerkInfo[args.ClerkId] = Info{args.Seq, reply.Value}
	kv.mu.Unlock()
	/*
