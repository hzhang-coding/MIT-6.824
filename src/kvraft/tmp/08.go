...
func (kv *KVServer) CheckState() {
	for kv.killed() == false {
		...
		if kv.isleader == true && isleader == false {
			for _, cond := range kv.waiting {
				cond.Broadcast()
			}
			kv.waiting = make(map[int64]*sync.Cond)
		}
...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	kv.waiting[args.ClerkId] = sync.NewCond(&kv.mu)
	...
	ch := make(chan int, 2)
	kv.waiting[args.ClerkId].Wait()

	delete(kv.waiting, args.ClerkId)
