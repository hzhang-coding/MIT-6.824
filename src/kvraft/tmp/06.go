...
func (kv *KVServer) CheckState() {
	for kv.killed() == false {
		...
		kv.mu.Lock()
		if kv.isleader == true && isleader == false {
			for _, wait := range kv.waiting {
				wait.cond.Broadcast()
			}
			kv.waiting = make(map[int64]Wait)
		}
	...
	for kv.killed() == false {
		...
		if msg.CommandValid == true {
			...
			if ok == false || op.Seq > info.seq{
				...
				_, ok = kv.waiting[op.ClerkId]
				if ok == true && kv.waiting[op.ClerkId].seq == op.Seq {
					kv.waiting[op.ClerkId].cond.Broadcast()
				}
			}
...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	_, ok = kv.waiting[args.ClerkId]
    if ok == true && args.Seq <= kv.waiting[args.ClerkId].seq {
		kv.mu.Unlock()
		return
    }
	kv.waiting[args.ClerkId] = Wait{args.Seq, sync.NewCond(&kv.mu)}
	...
	kv.waiting[args.ClerkId].cond.Wait()

	// defer fmt.Printf("%v exit!\n", args.Seq)
	/*
	go func() {
		time.Sleep(500 * time.Millisecond)
		kv.mu.Lock()
		_, ok := kv.wait[args.ClerkId]
		if ok == true {
			kv.wait[args.ClerkId].Broadcast()
		}
		kv.mu.Unlock()
	} ()
	*/
