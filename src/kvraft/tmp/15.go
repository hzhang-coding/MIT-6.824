...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	now := time.Now()
	duetime := now.Add(1000 * time.Millisecond)
	t1 := now.Add(50 * time.Millisecond)
	t2 := now.Add(150 * time.Millisecond)
	t3 := now.Add(500 * time.Millisecond)
    ...
    for time.Now().Before(duetime) == true {
		...
		if kv.term != term || kv.isleader == false {
			reply.Err = "WrongLeader"
			kv.mu.Unlock()
			return
		}
		...
		if time.Now().Before(t1) == true {
			time.Sleep(5 * time.Millisecond)
		} else if time.Now().Before(t2) == true {
			time.Sleep(10 * time.Millisecond)
		} else if time.Now().Before(t3) == true {
