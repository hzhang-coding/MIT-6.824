	...
	for kv.killed() == false {
		...
		if msg.CommandValid == true {
			...
			if ok == false || op.Seq > info.seq {
				...
				_, ok = kv.wait[op.ClerkId]
				if ok == true {
					kv.wait[op.ClerkId].Broadcast()
				}
			}
...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	_, ok = kv.hasStart[args.ClerkId]
	if ok == true && args.Seq <= kv.hasStart[args.ClerkId] {
		kv.mu.Unlock()
		return
	}
	...
	if ok == false {
		kv.wait[args.ClerkId] = sync.NewCond(&kv.mu)
	}
	kv.wait[args.ClerkId].Wait()
...
func StartKVServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int) *KVServer {
	...
	kv.wait = make(map[int64]*sync.Cond)
